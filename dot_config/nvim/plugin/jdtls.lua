local function set_java_bin()
    local path = vim.env.HOME .. "/.sdkman/candidates/java/"
    local command = "ls " .. path
    local handle = io.popen(command)
    local result = handle:read("*a")
    handle:close()

    local java_bin = "java" -- 默认值
    for folder in result:gmatch("[^\n]+") do
        if folder:find("^17") then
            java_bin = path .. folder .. "/bin/java"
            break
        end
    end
...
local function jdtls_setup(event)
    ...
    if cache_vars.java_bin == nil then
        cache_vars.java_bin = set_java_bin()
    end
